---
title: Installation
description: Install and set up HyprDots on Arch Linux.
---

HyprDots is designed to install cleanly on Arch Linux, link repo files into runtime config, and set up a polished Hyprland desktop.
...
> Back up existing directories such as `~/.config/hypr`, `~/.config/kitty`, and `~/.config/fish` before continuing. When you approve configuration setup, the installer replaces conflicting directories with symlinks to this repository.
...
- create the ignored local override files used by Hyprland
...
- `wireplumber`, `brightnessctl`, `hyprpicker`, `hyprshot`, `grim`, `slurp`, `wf-recorder`, `libnotify`
- `tesseract`, `tesseract-data-eng`, and `zbar` for OCR and barcode tools
...
4. press `Super` + `/` to open the built-in cheatsheet
